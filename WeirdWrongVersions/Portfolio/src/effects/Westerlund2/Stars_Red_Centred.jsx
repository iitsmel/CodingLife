...
export default function StarsRedCentred({ count = 300 }) {
  const positions = useMemo(() => {
    ...
    for (let i = 0; count > i; i++) {
      ...
      //down
      const dIII = 500 + r * Math.cos(theta) * Math.sin(phi) + (-100 + Math.random() * 500)
      const dIV = 200 + r * Math.cos(theta) * Math.sin(phi) + (-100 + Math.random() * 500)
      positions.push(dIII)
      positions.push(dIV)
    }
